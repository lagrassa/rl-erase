...
def fit_and_test_two_dim():
    #samples_file = np.load("dataset/samples_vary_"+var+".npy")
    #rewards_file = np.load("dataset/rewards_vary_"+var+".npy")
    samples_file, rewards_file = load_datasets("gp_pour_vary_two_only")
    dims = [1,2]
    assert(len(dims) == 2)
    num_training = 250
    ...
    plot_samples_and_rewards_2D(test_samples, test_rewards, predictions, gp=gp_2D, label="forces")
